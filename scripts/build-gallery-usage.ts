...
async function buildGalleryManifest(): Promise<void> {
  ...
  sortedFiles.forEach((file, index) => {
    ...
    lines.push(`import ${identifier} from "${importPath}";`);
    entries.push(`  ${identifier},`);
  });

  lines.push("", "const modules = [", ...entries, "] satisfies readonly (GallerySection | readonly GallerySection[])[];", "", "export default modules;", "");
